...
- **Set switcher (`SetTag`)**: the interactive variant reuses `CustomSelect` (`variant="tag"`), so it inherits that component's accessibility — an explicit `aria-label="Select set"`, `aria-haspopup="listbox"`, `aria-expanded`, `role="listbox"`/`role="option"` with `aria-selected`, full keyboard support (Arrow/Enter/Space/Escape/Tab), roving `tabIndex`, portal rendering, 44px option rows, and a `:focus-visible` ring. The current-vs-archived distinction is conveyed textually — via each dropdown option's "Current"/"Archived" sublabel and, for the selected set, an "Archived" suffix in the tag trigger itself — so the de-emphasized (muted) archived styling is never signaled by color alone. Note: the tag *trigger* is compact (below the 44px target) — a deliberate trade-off for a rarely-used, low-emphasis inline control; when only one set exists it is a non-interactive pill.

...
- **Set switcher (`SetTag`)**: the interactive variant reuses `CustomSelect` (`variant="tag"`), so it inherits that component's accessibility — an explicit `aria-label="Select set"`, `aria-haspopup="listbox"`, `aria-expanded`, `role="listbox"`/`role="option"` with `aria-selected`, full keyboard support (Arrow/Enter/Space/Escape/Tab), roving `tabIndex`, portal rendering, 44px option rows, a `:focus-visible` ring, and a dropdown chevron affordance. The visible chip matches the subtitle text height (trigger `height: 100%` of a `1em`-tall wrapper), but it still meets the **44×44** minimum tap target via an absolutely-positioned `::before` overlay (`height: 44px; min-width: 44px`, centered). The current-vs-archived distinction is conveyed textually — via each dropdown option's "Current"/"Archived" sublabel and, for the selected set, an "Archived" suffix in the tag trigger itself — so the de-emphasized (muted) archived styling is never signaled by color alone. When only one set exists it is a non-interactive pill.

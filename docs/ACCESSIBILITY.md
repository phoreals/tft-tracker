...
- **Set switcher (`SetTag`)**: the interactive variant reuses `CustomSelect` (`variant="tag"`), so it inherits that component's accessibility — an explicit `aria-label="Select set"`, `aria-haspopup="listbox"`, `aria-expanded`, `role="listbox"`/`role="option"` with `aria-selected`, full keyboard support (Arrow/Enter/Space/Escape/Tab), roving `tabIndex`, portal rendering, 44px option rows, a `:focus-visible` ring, and a dropdown chevron affordance. The visible chip is a compact 22px, but it still meets the **44×44** minimum tap target via an absolutely-positioned `::before` overlay (`height: 44px; min-width: 44px`, centered). The trigger shows only the set label (no "Archived" text), so the archived state is conveyed non-visually through the `SetTag` `aria-label` (e.g. "Select set — currently Set 17 (archived)"), which becomes the button's accessible name; it also stays visible in each dropdown option's "Current"/"Archived" sublabel. This ensures the de-emphasized (muted) archived styling is never signaled by color alone. When only one set exists it is a non-interactive pill.
- **Archived period pills**: on an archived set, all `DurationPill` period tags dim to muted grey (via `data-archived` on the page root). This is purely decorative de-emphasis — each pill's period text (e.g. "Set 17", "Week 3") stays fully legible, and the archived state itself is conveyed by the set tag's `aria-label` (above), not by the pills' color — so no information is lost for users who don't perceive the color change.

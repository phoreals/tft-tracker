...
**Match ID time scoping**: `getAllMatchIds` scopes to the active set with Riot's **`startTime`** query param (camelCase, epoch seconds) — TFT match-v1 follows match-v5's naming. Riot ignores unrecognised query params instead of erroring, so getting this name wrong does not fail loudly; it silently returns the player's whole career. It was `start_time` through 2026-08-28 and returned 911 IDs for a two-day-old set, every one fetched and discarded by the `tft_set_number` guard, exhausting one sync's entire budget and rate-limit allowance on a single player.

**Walk-off guard**: match IDs arrive newest-first, so `FOREIGN_RUN_LIMIT` (15) consecutive foreign-set matches means we have paged past the set boundary and everything older is foreign too. The batch loop stops there and reports `matchesRemaining: 0` — the unprocessed tail is old matches we don't want, not pending work. This is a structural backstop: it bounds the damage from any future scoping failure rather than trusting the query param to stay correct.
